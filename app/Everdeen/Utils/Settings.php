...
class Settings
{
    ...
    {
        $this->cookieName = _k('home_cookie_settings_name');
        $this->baseUrl = '';
        $this->baseName = '';
    ...
    public function resolveBase(Request $request)
    {
        $this->baseUrl = $request->input('_base_url', '');
        $this->baseName = $request->input('_base_name', '');
        $this->baseEmail = $request->input('_base_email', '');
    }
    ...
    public function fromCookie(Request $request)
    {
        ...
        if ($request->hasCookie($this->cookieName)) {
            $settings = json_decode($request->cookie($this->cookieName));
            if (!empty($settings)) {
                $this->setLocale($settings->locale);
                $this->setCountry($settings->country);
                $this->setTimezone($settings->timezone);
    ...
    {
        if (count($this->changingDataStore) > 0) {
            $cookieValue = AES::encrypt($this->toJson(), _k('home_encrypt_secret'));
            $response->headers->setCookie(
                new SymfonyCookie(
                    $this->cookieName, $cookieValue, AppConfig::MAX_COOKIE_EXPIRED_TIME,
